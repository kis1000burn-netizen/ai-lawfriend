...
for (const script of [...BUNDLED_VERIFY_SCRIPTS, "verify:aibeopchin-counter-argument-draft-engine-rc"]) {
...
for (const script of BUNDLED_VERIFY_SCRIPTS) {
  ...
  if (result.status !== 0) {
    console.error(`❌ Phase 63-F RC blocked: npm run ${script}`);
    process.exit(result.status ?? 1);
  }
}

console.log("✅ Phase 63-F Counter-Argument Draft Engine RC verified");
console.log("- 63-A Opponent Argument: LOCKED");
console.log("- 63-B Counter-Argument Candidate: LOCKED");
console.log("- 63-C Backfire Risk Check: LOCKED");
console.log("- 63-D Draft Paragraph Generator: LOCKED");
console.log("- 63-E Lawyer Review & Adoption Gate: LOCKED");
console.log("- Platform status: COUNTER_ARGUMENT_DRAFT_ENGINE_RC_LOCKED");
console.log(
  "verify:aibeopchin-counter-argument-draft-engine-rc PASS (Product Phase 63-F Counter-Argument Draft Engine RC)",
);
