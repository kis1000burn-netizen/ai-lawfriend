...
for (const script of [...BUNDLED_VERIFY_SCRIPTS, "verify:aibeopchin-judgment-backed-reasoning-rc"]) {
...
for (const script of BUNDLED_VERIFY_SCRIPTS) {
  ...
  if (result.status !== 0) {
    console.error(`❌ Judgment-backed Reasoning RC blocked: npm run ${script}`);
    process.exit(result.status ?? 1);
  }
}

console.log("✅ Judgment-backed Reasoning View RC verified");
console.log("- 64-A Judgment Reasoning Source Map: LOCKED");
console.log("- 64-B Judgment Reasoning View Builder: LOCKED");
console.log("- 64-C RC Gate: LOCKED");
console.log("- Platform status: JUDGMENT_BACKED_REASONING_RC_LOCKED");
console.log(
  "verify:aibeopchin-judgment-backed-reasoning-rc PASS (Judgment-backed Reasoning View RC)",
);
