  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
      <section className="rounded-2xl border bg-white p-6 shadow-sm">
        <div>
          <h2 className="text-lg font-bold text-aibeop-text">공유 범위 설정</h2>
        ...
        <div className="mt-6 grid gap-5">
          <label className="grid gap-2">
            <span className="text-sm font-semibold text-aibeop-subtle">공유 방식</span>
            <select
          ...
          <label className="grid gap-2">
            <span className="text-sm font-semibold text-aibeop-subtle">지정 변호사 ID</span>
            <input
              ...
              className="rounded-xl border px-3 py-2 text-sm outline-none focus:border-slate-500"
            />
            <span className="text-xs leading-5 text-aibeop-subtle">
          ...
          <label className="grid gap-2">
            <span className="text-sm font-semibold text-aibeop-subtle">공유 만료일</span>
            <input
              type="datetime-local"
          ...
          <div className="grid gap-3 rounded-2xl border bg-slate-50 p-4">
            <h3 className="text-sm font-bold text-aibeop-text">열람 허용 범위</h3>
          ...
          <div className="rounded-2xl border bg-white p-4">
            <h3 className="text-sm font-bold text-aibeop-text">공유 동의문</h3>
            <pre className="mt-3 whitespace-pre-wrap rounded-xl bg-slate-50 p-4 text-xs leading-6 text-aibeop-subtle">
              {CONSENT_TEXT}
            </pre>

            <label className="mt-4 flex items-start gap-3 text-sm text-aibeop-subtle">
              <input
                type="checkbox"
      ...
      <aside className="flex flex-col gap-6">
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          <h2 className="text-lg font-bold text-aibeop-text">현재 공유 상태</h2>
          <p className="mt-2 text-sm text-aibeop-muted">
            활성 공유 {activeShares.length}건 / 전체 {shares.length}건
          </p>

          {isLoading ? (
            <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-aibeop-subtle">
              공유 목록을 불러오는 중입니다.
            </div>
          ) : null}

          {!isLoading && shares.length === 0 ? (
            <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm leading-6 text-aibeop-muted">
...
function CheckboxRow({
  ...
  return (
    <label className="flex items-start gap-3 rounded-xl bg-white p-3 text-sm">
      ...
      />
      <span>
        <span className="block font-semibold text-aibeop-text">{title}</span>
        <span className="mt-1 block leading-5 text-aibeop-subtle">{description}</span>
      </span>
    </label>
  );
}
    ...
    <div className="rounded-2xl border p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 font-mono text-lg font-bold text-aibeop-text">{share.publicCode}</p>
        </div>

        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-aibeop-subtle">
          {share.status}
        </span>
      </div>

      <dl className="mt-4 grid gap-2 text-sm text-aibeop-muted">
        <div className="flex justify-between gap-3">
          <dt>공유 방식</dt>
          <dd className="font-medium text-aibeop-text">{share.shareMode}</dd>
        </div>
        <div className="flex justify-between gap-3">
          <dt>만료일</dt>
          <dd className="font-medium text-aibeop-text">
      ...
      <div className="mt-4 grid gap-2">
        <Link
          ...
          className="rounded-xl border px-3 py-2 text-center text-sm font-semibold text-aibeop-subtle hover:bg-slate-50"
        >
          상세 / 열람 이력 보기
        </Link>
