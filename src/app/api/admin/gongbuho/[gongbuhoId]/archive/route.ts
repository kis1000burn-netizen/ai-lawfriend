...
import { archiveGongbuhoPacket } from "@/features/gongbuho/gongbuho-packet.service";
...
export async function POST(_req: Request, context: RouteContext) {
  try {
    ...
    assertGongbuhoOperation(user, "ARCHIVE");
    const { gongbuhoId } = paramsSchema.parse(await context.params);

    const result = await archiveGongbuhoPacket({ gongbuhoId, actor: user });

    if (!result.alreadyArchived) {
      await writeGongbuhoAuditLog({
        actorUserId: user.id,
        event: "GONGBUHO_PACKET_ARCHIVED",
