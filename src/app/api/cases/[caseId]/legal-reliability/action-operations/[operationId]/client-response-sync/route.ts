...
import { syncClientResponseToLegalReliabilityOperationService } from "@/features/legal-reliability-action-operations/legal-reliability-action-operation-client-response-sync.service";
import {
  legalReliabilityActionOperationSchema,
  syncClientResponseToLegalReliabilityOperationInputSchema,
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const body = syncClientResponseToLegalReliabilityOperationInputSchema.parse(
      await request.json(),
    );

    const operation = await syncClientResponseToLegalReliabilityOperationService(
      currentUser,
      caseId,
      operationId,
      body,
