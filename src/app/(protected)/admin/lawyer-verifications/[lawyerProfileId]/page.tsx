  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">자격 검토 · {profile.user.name}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-800">계정</h2>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">이메일</dt>
              <dd className="text-slate-900">{profile.user.email}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">전화</dt>
              <dd className="text-slate-900">{profile.user.phone ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">User.status</dt>
              <dd className="text-slate-900">{profile.user.status}</dd>
            </div>
          </dl>
        </div>

        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-slate-800">변호사 등록·사무소</h2>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between gap-4">
            ...
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">지방변호사회</dt>
              <dd className="text-slate-900">{profile.barAssociation ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">사무소명</dt>
              <dd className="text-slate-900">{profile.officeName ?? "—"}</dd>
            </div>
            <div className="flex flex-col gap-1">
              <dt className="text-slate-500">사무실 주소</dt>
              <dd className="text-slate-900">{profile.officeAddress ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">사무실 전화</dt>
              <dd className="text-slate-900">{profile.officePhone ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-slate-500">웹사이트</dt>
              <dd className="break-all text-slate-900">{profile.websiteUrl ?? "—"}</dd>
            </div>
            <div className="flex flex-col gap-1">
              <dt className="text-slate-500">전문 분야 메모</dt>
              <dd className="text-slate-900">{profile.specialtiesNote ?? "—"}</dd>
            </div>
          </dl>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-slate-800">가입 시 무결 서약 · 접속 패턴(참고)</h2>
        ...
        <dl className="mt-3 space-y-2 text-sm">
          <div className="flex flex-wrap justify-between gap-4">
            <dt className="text-slate-500">무결 서약 접수</dt>
          ...
          <div className="flex flex-wrap justify-between gap-4">
            <dt className="text-slate-500">서약 문구 버전</dt>
            <dd className="break-all text-slate-900">
              {profile.integrityAttestationVersion ?? "—"}
            </dd>
          </div>
          <div className="flex flex-wrap justify-between gap-4">
            <dt className="text-slate-500">가입 접속 지문(IP)</dt>
          ...
          <div className="flex flex-wrap justify-between gap-4">
            <dt className="text-slate-500">동일 접속 지문 프로필</dt>
          ...
          <div className="flex flex-col gap-1">
            <dt className="text-slate-500">User-Agent 앞부분</dt>
            <dd className="break-all text-xs text-slate-900">{profile.signupRiskUserAgent ?? "—"}</dd>
          </div>
        </dl>
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-4">
        <h2 className="text-sm font-semibold text-slate-800">제출 증빙</h2>
        <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-slate-600">
          ...
          <span>
            <span className="mr-1 rounded bg-slate-200 px-1.5 py-0.5 font-semibold text-slate-800">
              Private
            </span>
            storageKey·신규 업로드
          </span>
        ...
        <p className="mt-2 text-xs leading-relaxed text-slate-600">
          「열기」는 <strong className="text-slate-800">관리자 세션</strong>이 있는 경우에만 동작하며, 요청 시{" "}
          <strong className="text-slate-800">LAWYER_VERIFICATION_DOCUMENT_ACCESS</strong> 감사로그가
          남습니다. 링크만 타인에게 넘겨도 외부인은 볼 수 없습니다. 민감 자료는 DB에 공개 URL을 두지 않고{" "}
          <strong className="text-slate-800">storageKey + 짧은 만료 signed URL</strong>로 보관하는 것이
          기준입니다. <strong className="text-slate-800">fileUrl</strong>만 있는 행은{" "}
          <strong className="text-slate-800">Legacy</strong>로 표시되며(P4 이관 스크립트 대상), 이관 후에도
          <strong className="text-slate-800"> fileUrl</strong> 컬럼은 바로 삭제하지 않습니다.
        </p>
        {profile.verificationDocuments.length === 0 ? (
          <p className="mt-2 text-sm text-slate-500">등록된 파일이 없습니다.</p>
        ) : (
              ...
              >
                <div>
                  ...
                  {hasStorage ? (
                    <span className="ml-2 text-xs text-slate-500">
                      {doc.migratedAt ? (
                        ...
                        <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-800">
                          Private
                        </span>
                      )}
                    </span>
                  ...
                  {hasStorage && doc.fileUrl?.trim() ? (
                    <span className="ml-2 text-xs text-slate-400">(+ legacy fileUrl 필드 보존)</span>
                  ) : null}
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
        <h2 className="text-sm font-semibold text-slate-800">최근 심사 기록</h2>
        <dl className="mt-2 space-y-1">
          <div>
            <span className="text-slate-500">reviewedAt</span> —{" "}
            {profile.reviewedAt ? new Date(profile.reviewedAt).toLocaleString("ko-KR") : "—"}
          </div>
          <div>
            <span className="text-slate-500">reviewedBy</span> —{" "}
            {profile.reviewedBy
              ? `${profile.reviewedBy.name} (${profile.reviewedBy.email})`
              : "—"}
          </div>
          <div className="mt-2">
            <span className="text-slate-500">rejectionReason / 안내</span>
